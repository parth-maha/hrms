...
const ReferJobModal = ({ open, onClose, jobId }: ReferJobModalProps) => {
  ...
  const {
    ...
    defaultValues: { ToName: "", ToEmail: "", ToCv: null as File | null, ReferredBy: empId },
  });
        ...
        <div className="w-full">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (file) setValue("ToCv", file, { shouldValidate: true });
            }}
            className="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 focus:outline-none p-2"
          />
          {errors.ToCv && (
            <p className="text-red-500 text-xs mt-1">CV is required</p>
          )}
        </div>
